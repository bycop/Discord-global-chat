...
// to actually make it look good currently. It could probs me half the size or less. again, I know
// 
    ...
    bot.guilds.cache.forEach(async (guild) => {
      ...
      channelMessages.forEach(async (msg) => {
        ...
        if (message.author.id !== "215534700284870658") {
          try {
            if (msg?.embeds[0]?.footer?.text?.includes(args[0]) && msg?.embeds[0]?.author?.name.includes(message.author.tag))
              await msg.delete();
          } catch (error) {
            console.error(error);
          } return;
      ...
      if (guild == message.guild) {
        ...
        channelMessages.forEach(async (msg) => {
          try {
            ...
            if (msg?.embeds[0]?.footer?.text?.includes(messageID) && msg?.embeds[0]?.author?.name.includes(message.author.tag)) {
